        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300 font-semibold">Notifikasi</div>
                        <div class="border-t border-gray-200 dark:border-gray-600"></div>
                        @forelse($notifications as $notification)
                            <x-dropdown-link href="{{ $notification->url ?? '#' }}">
                                {{ $notification->message }}
                                <span class="block text-xs text-gray-500">{{ $notification->created_at->diffForHumans() }}</span>
                            </x-dropdown-link>
                        @empty
                            <div class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">Tidak ada notifikasi baru.</div>
                        @endforelse
                    </x-slot>
                </x-dropdown>
            </div>
