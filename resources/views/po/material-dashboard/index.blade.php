    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Monitor dan kelola material tracking dengan mudah</p>
            </div>
...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <!-- Material Summary Cards (Diterima vs Digunakan vs Tersisa) -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 truncate">Total Material Diterima</dt>
                                <dd class="text-lg font-medium text-gray-900">{{ number_format($materialSummary->sum('total_received'), 0) }} unit</dd>
                            </dl>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 truncate">Total Material Digunakan</dt>
                                <dd class="text-lg font-medium text-gray-900">{{ number_format($materialSummary->sum('total_used'), 0) }} unit</dd>
                            </dl>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 truncate">Total Material Tersisa</dt>
                                <dd class="text-lg font-medium text-gray-900">{{ number_format($materialSummary->sum('remaining_stock'), 0) }} unit</dd>
                            </dl>
                        </div>
                    </div>
                </div>
            ...
            <!-- Persentase Penggunaan Keseluruhan -->
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 truncate">Tingkat Penggunaan</dt>
                                <dd class="text-lg font-medium text-gray-900">
                                    @php
                                        $totalReceived = $materialSummary->sum('total_received');
                                        $totalUsed = $materialSummary->sum('total_used');
                                        $overallUsagePercentage = $totalReceived > 0 ? round(($totalUsed / $totalReceived) * 100, 1) : 0;
                                    @endphp
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg mb-6">
                <div class="p-6 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-medium text-gray-900">Ringkasan Material (Diterima vs Digunakan vs Tersisa)</h3>
                        <div class="flex items-center space-x-3">
                            <div class="text-sm text-gray-500">{{ $materialSummary->count() }} material</div>
                            <a href="{{ route('po.material-stock.index') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">
                                Lihat Detail Stock →
                            </a>
                        </div>
                    </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Stock Diterima</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sudah Digunakan</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Stock Tersisa</th>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <div class="text-sm text-gray-500">{{ $material['material_category'] }} ({{ $material['unit'] }})</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-green-900">{{ number_format($material['total_received'], 0) }}</div>
                                        <div class="text-sm text-gray-500">{{ $material['unit'] }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-red-900">{{ number_format($material['total_used'], 0) }}</div>
                                        <div class="text-sm text-gray-500">{{ $material['unit'] }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ number_format($material['remaining_stock'], 0) }}</div>
                                        <div class="text-sm text-gray-500">{{ $material['unit'] }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-16 bg-gray-200 rounded-full h-2 mr-2">
                                                <div class="bg-blue-600 h-2 rounded-full" style="width: {{ min(100, $material['usage_percentage']) }}%"></div>
                                            </div>
                                            <span class="text-sm font-medium text-gray-900">{{ $material['usage_percentage'] }}%</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            @if($material['status']['color'] === 'red') bg-red-100 text-red-800
                                            @elseif($material['status']['color'] === 'yellow') bg-yellow-100 text-yellow-800
                                            @elseif($material['status']['color'] === 'orange') bg-orange-100 text-orange-800
                ...
                <div class="text-center py-8">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">Belum Ada Data Material</h3>
                    <p class="mt-1 text-sm text-gray-500">Data material akan muncul setelah ada penerimaan material.</p>
                </div>
                @endif
            </div>
