                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            @if($transaction->type === 'pengembalian')
                                                @if($transaction->return_destination)
                                            ...
                                            <div class="text-xs">
                                                @if($transaction->type === 'penerimaan')
                                                    @if($transaction->delivery_order_no)
                                                        <div><span class="text-gray-500">DO:</span> {{ $transaction->delivery_order_no }}</div>
                                                    @endif
                                                    @if($transaction->delivery_note_no)
                                                        <div><span class="text-gray-500">DN:</span> {{ $transaction->delivery_note_no }}</div>
                                                    @endif
                                                @elseif($transaction->type === 'pengembalian' && $transaction->delivery_return_no)
                                                    <div><span class="text-gray-500">DR:</span> {{ $transaction->delivery_return_no }}</div>
                                                @else
